...
public class AlignmentSafetyTest {
    ...
    @Test
    public void safeJustifyCenterFallsBackOnMainAxisOverflow() {
        TaffyTree tree = new TaffyTree();
        TaffyStyle rootStyle = new TaffyStyle();
        rootStyle.display = TaffyDisplay.FLEX;
        rootStyle.size = new TaffySize<>(TaffyDimension.length(50f), TaffyDimension.length(50f));
        rootStyle.justifyContent = AlignContent.SAFE_CENTER;
        NodeId child = tree.newLeaf(new TaffyStyle());
        TaffyStyle childStyle = tree.getStyle(child);
        childStyle.size = new TaffySize<>(TaffyDimension.length(100f), TaffyDimension.length(10f));
        NodeId root = tree.newWithChildren(rootStyle, child);
